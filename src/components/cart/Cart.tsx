import type { CartItem } from "./useCart"
import { TELEFONO } from "../../config"

interface Props {
  items: CartItem[]
  total: () => number
  totalItems: () => number
  update: (id: string, delta: number) => void
  msjWpp: (phone: string) => string
}

export function Cart({ items, total, totalItems, update, msjWpp }: Props) {
  return (
    <div className="bg-white border border-primary-light rounded-xl p-3 flex flex-col gap-3">

      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-primary-dark tracking-wide uppercase">
          Mi pedido
        </span>
        <span className="bg-primary text-white text-xs px-2 py-0.5 rounded-full font-medium">
          {totalItems()}
        </span>
      </div>

      {items.length === 0 ? (
        <p className="text-xs text-gray-400 text-center py-3">
          Sin productos
        </p>
      ) : (
        <div className="flex flex-col gap-2 max-h-48 overflow-y-auto">
          {items.map(({ product, quantity }) => (
            <div key={product.id} className="flex items-center gap-2 text-xs">
              <span className="flex-1 text-dark truncate">{product.nombre}</span>
              <div className="flex items-center gap-1">
                <button
                  onClick={() => update(product.id, -1)}
                  className="w-5 h-5 bg-gray-100 rounded text-gray-600 hover:bg-gray-200 flex items-center justify-center"
                >−</button>
                <span className="w-4 text-center font-medium text-dark">
                  {quantity}
                </span>
                <button
                  onClick={() => update(product.id, 1)}
                  className="w-5 h-5 bg-gray-100 rounded text-gray-600 hover:bg-gray-200 flex items-center justify-center"
                >+</button>
              </div>
            </div>
          ))}
        </div>
      )}

      <div className="border-t border-gray-100 pt-2 flex justify-between text-xs">
        <span className="text-gray-500">Total</span>
        <span className="font-medium text-dark">
          ${total().toLocaleString("es-AR")}
        </span>
      </div>

       <a
          href={items.length > 0 ? msjWpp(TELEFONO) : undefined}
          target="_blank"
          rel="noreferrer"
          className={`flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-medium text-white transition-opacity
            ${items.length > 0 ? "bg-[#25D366] hover:opacity-90" : "bg-gray-300 pointer-events-none"}`}
       >
        <svg width="13" height="13" viewBox="0 0 24 24" fill="white">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
        </svg> 
        Enviar por WhatsApp 
        </a>
   </div>
    );
}